...
void circle(float radius,float u,float v)
{
...
void display1()
{
	...
	drawText(25.0, 200.0, 0, 0, 1, d, "Sir Prashanth ");
...
{
	int j,k,t=0,f=0;
	for(k=0;k<=200;k++)//to move sat near mars
	{
		if(exitflag==1)break;
		glClearColor(0.0 ,0.0 ,0.0,1.0);
		glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);
	...
	for(k=0;k<=200;k++)//diagonally move sat
	{
		...
		circle(75,250,250);
		glPushMatrix();
		glScalef(1-(0.0015*k),1-(0.0015*k),1);
	...
	for(k=0;k<2;k++)//rotate around mars
	{
		...
		for(j=0;j<360;j++)
		{
			...
			solar_panel11();
			glPopMatrix();
			drawText1(150, 425, 1, 1, 0, 0, "MISSION SUCCESSFUL...!!");
